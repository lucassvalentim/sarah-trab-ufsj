...
class ControleProfissionalSaude:
    ...
    def pesquisar(self, nome=None, cpf=None, crm=None, id=None):
        if id is not None:
            ...
            condition = f"cpf = '{cpf}'"
            rows = self.persistencia.fetch('ProfissionalSaude', ['id', 'nome', 'idade', 'cpf', 'sexo', 'localidade', 'especializacao', 'crm', 'formacao', 'tempoAtividade', 'convenios', 'precoConsulta'], condition)
            if rows:
                row = rows[0]
